...
hprop_plutus_certifying_withdrawing = H.integration . H.runFinallies . H.workspace "chairman" $ \tempAbsBasePath' -> do
  ...
  txin5 <- H.noteShow $ Map.keys utxo5 !! 0
  ...
  void $ H.execCli' execConfig
    ...
    , "--address", plutusStakingAddr
    , "--testnet-magic", show @Int testnetMagic
    , "--out-file", work </> "plutus-staking-script-delegation-rewards-2.json"
    ]

  stakingRewardsJSON2 <- H.leftFailM . H.readJsonFile $ work </> "plutus-staking-script-delegation-rewards-2.json"
  delegsAndRewardsMapScriptRewards2 <- H.noteShowM $ H.jsonErrorFail $ J.fromJSON @DelegationsAndRewards stakingRewardsJSON2
  let delegsAndRewardsScriptRewards2 = mergeDelegsAndRewards delegsAndRewardsMapScriptRewards2
      stakingScriptRewardsAddrInfo2 = filter (\(sAddr,_,_) -> plutusStakingAddr == T.unpack (serialiseAddress sAddr)) delegsAndRewardsScriptRewards2
      (_, scriptRewards2, _) = head stakingScriptRewardsAddrInfo2

  case scriptRewards2 of
    Nothing -> H.failMessage callStack "Plutus staking script has no rewards"
    Just rwds -> rwds === 0
